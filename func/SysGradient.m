function [dsys_da, dsys_dv]=SysGradient(ps,V,ueig,veig,lambda)
...
% 
dsys_da = real(dsys_da)./imag(lambda) - (real(lambda)/((imag(lambda))^2)).*imag(dsys_da);
dsys_dv = real(dsys_dv)./imag(lambda) - (real(lambda)/((imag(lambda))^2)).*imag(dsys_dv);
